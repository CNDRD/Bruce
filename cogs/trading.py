...
class Trading(commands.Cog):
    ...
    async def _trade(
        ...
        currently_owns = db.child("trading").child(inter.author.id).get().val()
        ...
        match operation:
            case 'Buy':
                ...
                if stock in currently_owns:
                    return await inter.edit_original_message(content=f"You already own shares of {stock}")

                buying = yf.Ticker(stock)
                current_price = buying.info.get('currentPrice')
                ...
                db.child("users").child(inter.author.id).update({"money": (user_money - buying_cost)})
            ...
            case 'Sell':
                ...
                selling = yf.Ticker(stock)
                current_price = selling.info.get('currentPrice')
                ...
                db.child("users").child(inter.author.id).update({"money": (user_money + selling_cost)})

                msg = f"Successfully sold *{amount_bought}* stocks of **{stock}** for a __{profit_loss}__ of *{round(outcome_money, 2)}* shekels"
                return await inter.edit_original_message(content=msg)

            case "Show":
                msg = "__Your current stonks sir:__\n\n"
                tickers = yf.Tickers(' '.join(currently_owns))

                for stonk in currently_owns:
                    current_price = tickers.tickers[stonk].info.get('currentPrice')
                    ...
                    msg += f"**{stonk}** - " \
                           f"Current Price: __*{current_price}*__ | " \
                           f"Bought at: __*{bought_price}*__ | " \
                           f"Profit: __*{round(profit, 2)}*__ " \
