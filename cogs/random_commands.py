...
class RandomCommands(commands.Cog):
    ...
    async def vanish(self, ctx):
        ...
        # Kicks the clown who typed the command
    ...
    async def ping(self, ctx):
        ...
        # Reply with a message with Bots latency in ms
    ...
    async def clear(self, ctx, amount: int = 1):
        ...
        # Clear 'amount' of messages +1 to account for the command itself
    ...
    async def backup_db(self, ctx):
        ...
        # Automatically backs up the whole Database and sends it to me
        # because only i can use this command lmao
    ...
    async def manually_add_to_db(self, ctx, uid = None):
        ...
        # What even is the point of this without the ID dude
        if uid is None:
            await ctx.message.add_reaction('❌')
            return

        # Get the desired member
    ...
    async def sheriff(self, ctx, e=None):
        ...
        # If no emote is provided defaults to this
        # supports using the name of the emoji for plebs without nitro
        # for animated amoji
        if e is None:
            e = '🖕'
        elif '<' in e and '>' in e:
    ...
    async def coinflip(self, ctx, heads: str = "Heads", tails: str = "Tails"):
        ...
        # Unnecesarily complicated but also fuck you
        why = np.random.randint(0,16528) % 2
    ...
    async def emoji_square(self, ctx, e=None, size: int = 5):
        ...
        # If no emote is provided defaults to '🖕'
        # supports using the name of the emoji for plebs without nitro
        # for animated amoji
        if e is None:
            e = '🖕'
    ...
    async def code(self, ctx):
        ...
        # Just sends the Repo link in a nice embed
    ...
    async def cicina(self, ctx):
        ...
        # Hardcore calculations therefore a function is used
        cicina = get_cicina()

        # Last date that cicina was tried
        cicinaLast = db.child('users').child(uid).child('cicina_last').get().val()
        if cicinaLast is None: cicinaLast = 0

        # Longest cicina that the user ever had
        ...
        # First cicina average calculations
        if cicinaCount is None:
            new_count = 1
            new_avg = cicina

        # Any other cicina average calculations
        else:
    ...
    async def quote(self, ctx, *quote):
        ...
        # If quote starts with '-' it is not a quote but a quote ID
        # this deletes the quote with that ID
        # If the quote starts with an actual '-' fuck you and try again..
        if quote.startswith('-'):
            db.child('quotes').child(quote).remove()
            await ctx.message.add_reaction('✅')
            return

        # Get the Firebase key so it can be shown to the user
        ...
        try:
            # Wait 10s for the possible reply from the author with the quote author
            quote_by = await self.client.wait_for('message', timeout=10.0, check=check)
        except:
            # No author was given; set up the quote without an author and a time
            ...
            try:
                # Wait 20s for the possible reply from the author with the quote date
                quote_when = await self.client.wait_for('message', timeout=20.0, check=check)
            except:
                # No date was given; set up the quote without a time
                await mess.edit(content=f'Final quote: \n***{quote}*** *-{quote_by.content}*\nQuote can be deleted using this ID: `{quote_id}`')
                data = {"quote": quote, "by": quote_by.content, "when": "unknown"}
            else:
                # Quote, author & date provided
                # Send deletion details
